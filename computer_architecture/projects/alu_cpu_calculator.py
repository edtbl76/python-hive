...
class UltraSuperCalculator:
    ...
    def store_value_to_register(self, value_to_store):
        ...
        # value to store is binary, so provide int value to
        # registers
        self.number_registers[self.numbers_index] = int(value_to_store, 2)
        print(f"Value: {value_to_store} ({int(value_to_store, 2)}) stored ain Register: {self.numbers_index} ")

        # increment numbers index so next stored value will be
        # stored in the next available register
        self.numbers_index += 1
    ...
    def get_last_calculation(self):
        # decr history index to look back by 1
